...
# 5000개의 영화 데이터를 로드
rawdf = pd.read_csv('C:/ITStudy/kyu_kafka_practice/kafka_practice/dfs/tmdb_5000_movies.csv')

# 1. 랜덤하게 100개 행을 뽑아서 고객 A의 시청 목록 A.csv로 생성
customer_a_watchlist = rawdf.sample(n=100)
watchlist_filename = f'{myconfig.customer_name3}.csv'
customer_a_watchlist.to_csv(f'C:/ITStudy/kyu_kafka_practice/kafka_practice/dfs/{watchlist_filename}', index=False)
...
favorites_filename = f'{myconfig.customer_name3}_favorites.csv'
customer_a_favorites.to_csv(f'C:/ITStudy/kyu_kafka_practice/kafka_practice/fav_dfs/{favorites_filename}', index=False)
...
try:
    ...
    producer.send(myconfig.customer_name3, key=most_watched_genre, value=favorites_filename)
    print(f"{myconfig.customer_name3}님이 가장 좋아하시는 영화를 추출했습니다.")
